...
bool inite_g_gps_position_measurement = false;
...
double K=15.9;//方向盘转角比车轮转角/////////////////////////////////////////////////////////////////////////////
double w_limit=0.05;
int seq_duan = 2100;
double per = 0.945;//测量轮角速度到轮速的误差比例，半径误差
...
void gpsCallback(const nav_msgs::OdometryConstPtr& input)// POS VEL YAW
{
    ...
    else
    {
        ...
        {//将四元数转换为欧拉角
            ...
            g_gps_heading_measurement_changed = false;
...
int main(int argc, char **argv)
{
    ...
    odom_akermanbs_pub = node.advertise<nav_msgs::Odometry>("/akermangpsodometry", 10000);//pub出估计的结果
