...
int main(int argc, char **argv)
{
    ...
    ros::init(argc, argv, "akermanlidar");
    ros::NodeHandle node;
    ros::NodeHandle priv_node("~");

    odom_akermanbs_pub = node.advertise<nav_msgs::Odometry>("/akermanlidarodometry", 10000);//pub出估计的结果
    ...
    ros::Subscriber LIDAR_sub = node.subscribe("/lidar_odom", 10000, lidarCallbacknew);
