...
struct SModelParameter : KalmanFilterModelParameterBase
{
    SModelParameter() //构造函数
    {
        ...
        this->process_noise.middleRows(0, 2) *= 10.0;//pos
        this->process_noise.middleRows(2, 2) *= 10.0;//v
        this->process_noise.middleRows(4, 1) *= 1.0;//yaw
        this->process_noise.middleRows(5, 2) *= 10.0;//a
        this->process_noise.middleRows(7, 1) *= 1.0;//w
        this->process_noise.middleRows(8, 2) *= 0.01;//a_bias
        this->process_noise.middleRows(10, 1) *= 0.008;//g_bias
    }
     //重载
...
public:
    ...
    )//属于类的函数
    {
        ...
        // position
        xsig_prediction.middleRows(0, 2) = p + v * delta;//零阶保持
