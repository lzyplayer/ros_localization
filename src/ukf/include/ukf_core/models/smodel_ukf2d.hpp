...
struct SModelParameter : KalmanFilterModelParameterBase
{
    SModelParameter() //构造函数
    {
        ...
        this->process_noise.middleRows(0, 2) *= 0.05;//pos
        this->process_noise.middleRows(2, 2) *= 0.05;//v
        this->process_noise.middleRows(4, 1) *= 0.05;//yaw
     ...
     //重载
    SModelParameter(double pos, double v, double yaw, double a, double w)
    {
        this->process_noise = Eigen::MatrixXd::Identity(11, 11);
        this->process_noise.middleRows(0, 2) *= pos;//pos
        this->process_noise.middleRows(2, 2) *= v;//v
        this->process_noise.middleRows(4, 1) *= yaw;//yaw
        this->process_noise.middleRows(5, 2) *= a;//a
        this->process_noise.middleRows(7, 1) *= w;//w
        this->process_noise.middleRows(8, 2) *= 1e-6;//ab
        this->process_noise.middleRows(10, 1) *= 1e-6;//wb
    }
