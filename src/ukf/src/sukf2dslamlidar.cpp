...
Eigen::Matrix<double, 1, 1> measurement2_slam;//yaw
...
void SlamsigCallback(const nav_msgs::OdometryConstPtr& input)//记录状态信息
{
    ...
    Zpre_slam Zs;
    // Zs.x = g_filter_s.x_pre(0);//prediction;
    // Zs.y = g_filter_s.x_pre(1);
    // Zs.t = g_filter_s.x_pre(4);
    Zs.x = outputstate_s_(0);
    Zs.y = outputstate_s_(1);
    Zs.t = outputstate_s_(4);
    Zs.stamp = t2_;//获取时
...
void SlamCallback(const nav_msgs::OdometryConstPtr& input)//两个时间戳，bag原始的时间戳以及计算时间长度，得到解算完的对应bag的时间
{
    ...
    else
    {
        ...
        if(!M_slam.empty())
        {
            ...
            // 0,input->pose.covariance[1] * input->pose.covariance[1];//鲁棒
...
void update(double timestamp, ros::Time stamp)//来什么量测，用什么来更新
{
    ...
    boost::mutex::scoped_lock lock(inputmutex);////////////////////////////////////////////////////////////////////////////////////////////////////
    {
        ...
        if(use_slam) 
        {
            ...
            //pos
            // measurement1 << g_filter_s.x_pre(0), g_filter_s.x_pre(1);
            measurement1 << outputstate_s_(0), outputstate_s_(1);
            ...
            //yaw
            // measurement2 << g_filter_s.x_pre(4);
            measurement2 << outputstate_s_(4);
